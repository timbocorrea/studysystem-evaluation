-- STUDYSYSTEM-2026
-- FASE 14K-8
-- Corrige colisao de nomes em RPCs de pool/question_bank.
--
-- Estrategia:
-- - Renomeia a variavel de loop para quiz_q.
-- - Usa qb como alias da tabela question_bank.
-- - Preserva assinaturas, grants e comportamento server-side.
-- - Nao cria dados, nao altera RLS e nao expoe gabarito.
...
begin
  ...
  if v_has_static_questions then
    ...
    for quiz_q in
      ...
      where quiz_id = p_quiz_id
    loop
      v_points := coalesce(quiz_q.points, 10);
      v_total_points := v_total_points + v_points;

      if p_answers ? quiz_q.id::text then
        v_option_id := (p_answers ->> quiz_q.id::text)::uuid;

        select qo.is_correct
          into v_is_correct
        from public.quiz_options qo
        where qo.id = v_option_id
          and qo.question_id = quiz_q.id;
