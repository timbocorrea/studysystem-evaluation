-- STUDYSYSTEM-2026
-- FASE B — scoring server-side para tentativas static em subset configurado.
--
-- Esta migration redefine somente submit_quiz_attempt. O caminho pool
-- permanece compatível com a implementação anterior.
  ...
  if v_has_static_questions then
    ...
    if v_answers_count <> v_expected_count then
      raise exception 'Complete quiz attempt required: expected %, got %',
        v_expected_count, v_answers_count;
    end if;
    ...
    loop
      ...
      if not found then
        raise exception 'Question % does not belong to quiz %', v_q_id, p_quiz_id;
      end if;
    ...
    if v_total_points <= 0 then
      raise exception 'Quiz has no scorable points';
    end if;
  else
    ...
    where (v_course_id is null or qb.course_id = v_course_id)
    ...
    if v_answers_count > v_expected_count then
      raise exception 'Too many answers submitted. Expected at most %, got %',
        v_expected_count, v_answers_count;
    end if;
    ...
    loop
      ...
      select qb.points, qb.course_id, qb.lesson_id, qb.module_id
